...
const RequestedInformationSection = ({ requestedInformation }) => {
  ...
  // Handle case where it might be an array (legacy support) or just the info object
  const data = useMemo(() => {
    return Array.isArray(requestedInformation)
      ...
      <div className={styles.section} dir={dir}>
        <div className={styles.empty}>
          <p className={styles.emptyTitle}>
            {t("requestedInfo.emptyTitle", "No information available right now")}
          </p>
        </div>
      </div>
    );
  ...
  // Keys in a specific order
  const priorityFields = [
    { key: "number", label: t("requestedInfo.fields.number", "Order number") },
    { key: "title", label: t("requestedInfo.fields.title", "Title") },
    { key: "case_type_name", label: t("requestedInfo.fields.case_type_name", "Service type") },
    { key: "status_label", label: t("requestedInfo.fields.status_label", "Status"), isStatus: true },
    { key: "court_name", label: t("requestedInfo.fields.court_name", "Court") },
    { key: "case_reference_no", label: t("requestedInfo.fields.case_reference_no", "Reference number") },
    { key: "created_at", label: t("requestedInfo.fields.created_at", "Created at") },
    { key: "updated_at", label: t("requestedInfo.fields.updated_at", "Updated at") },
  ];

  // Exclude already handled / nested objects
  ...
  const dynamicFields = Object.entries(data)
    ...
    .map(([key, value]) => ({
      label: t(`requestedInfo.fields.${key}`, prettifyKey(key)),
      value,
    }));

  const allGeneralFields = [...priorityFields.map((f) => ({ ...f, value: data[f.key] })), ...dynamicFields].filter(
  ...
  return (
    <div className={styles.section} dir={dir}>
      ...
      <div className={styles.card}>
        <h3 className={styles.cardTitle}>{t("tabs.info", "General information")}</h3>
      ...
      {data.client && (
        <div className={styles.card}>
          <h3 className={styles.cardTitle}>{t("requestedInfo.cards.client", "Client information")}</h3>
          <div className={styles.infoGrid}>
            <div className={styles.infoItem}>
              <span className={styles.infoLabel}>{t("requestedInfo.fields.name", "Name")}</span>
              <span className={styles.infoValue}>{data.client?.name || "---"}</span>
            </div>
            <div className={styles.infoItem}>
              <span className={styles.infoLabel}>{t("requestedInfo.fields.phone", "Phone")}</span>
              <span className={styles.infoValue}>{data.client?.phone || "---"}</span>
            </div>
            <div className={styles.infoItem}>
              <span className={styles.infoLabel}>{t("requestedInfo.fields.email", "Email")}</span>
      ...
      {(data.assigned_user || (data.users && data.users.length > 0)) && (
        <div className={styles.card}>
          <h3 className={styles.cardTitle}>{t("requestedInfo.cards.assignee", "Assigned staff")}</h3>
