...
const Sidebar = ({ hideOnMobile = false, variant = "default" }) => {
  ...
  // ✅ Use the same data shape as <User /> component (user.account.*)
  // while still supporting any older/other shapes to avoid breaking UI.
  const account = useMemo(() => user?.account ?? user ?? {}, [user]);

  const userId = account?.id ?? user?.id;
  const userName = account?.name ?? user?.name ?? user?.username ?? "...";
  const userEmail = account?.email ?? user?.email ?? "";
  const userPhone = account?.phone ?? user?.phone ?? "";
  const avatarSrc =
    account?.avatar_url || user?.avatar || user?.image || "/assets/imgs/client.png";
  ...
  return (
    <aside className={sidebarClassName} dir={dir}>
          ...
          <div className={styles.profileBody}>
            <div className={styles.profileAvatar}>
              <img src={avatarSrc} alt="User avatar" className={styles.profileImage} />
               <Link to={`${base}/account-settings`} className={styles.editButton} aria-label="Edit profile">
          ...
          <div className="logo_container">
            <div className="user flex items-center justify-between">
              <div className="flex items-center justify-between gap-2">
                {/* ✅ avatar from account.avatar_url */}
              ...
              {/* ✅ safe user id */}
              <Link to={`${URL_OUT_BASE}${base}/invite-friend/${userId || ""}`}>
                <img
          ...
          <div className={styles.userHeader}>
            <div className={styles.userInfo}>
              ...
              <div className="flex gap-2">
                ...
                />
                {/* ✅ show same style like User component */}
                <p className={styles.lawyer}>{userName} /محامى</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className={`${styles.menu} ${isProfileVariant ? styles.menuProfile : ""}`}>
        ...
        <Link
          to={`${URL_OUT_BASE}${base}/invite-friend/${userId || ""}`}
          className={`${styles.menuItem} ${isProfileVariant ? styles.menuItemProfile : ""}`}
        >
          <div className={`flex items-center gap-3 ${styles.menuContent}`}>
            <span className={styles.menuIcon}>
